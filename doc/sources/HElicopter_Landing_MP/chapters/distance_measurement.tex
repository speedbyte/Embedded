...
\chapter{Used Distance Measurement Sensors}
...
We tried to establish a distance measurement with GP2Y0A60SZLF Infrared Sensor on a Pololu Carrier Board. The Measuring distance of that sensor is 10 cm to 150 cm. For detailed technical values, please see the sensor chapter or the datasheet.
...
\begin{figure}[H]
	\centering\includegraphics[width=0.8\textwidth]{fig/adc_ir_wiring}
...
\subsection{Connecting the sensor with I$^2$C}
\label{sec:lascon}

Because there is no proper input for a PWM signal, we used the I$^2$C Bus to connect the laser sensor. As the sensor does not support fast I$^2$C mode, which we need, we decided to use the second I$^2$C on the Raspberry Pi. This bus is originally located on the camera port with an FPC (flexible printed circuit) Connector. We managed it to redirect it to the normal Pins on the board. See Chapter \ref{sec:setupi2c0}.

\begin{figure}[H]
	\centering\includegraphics[width=0.8\textwidth]{fig/lidar_wiring}
...
\begin{figure}[H]
	\centering\includegraphics[width=0.75\textwidth]{fig/laser_mess}
...
\begin{figure}[H]
	\centering\includegraphics[width=0.5\textwidth]{fig/angles}
