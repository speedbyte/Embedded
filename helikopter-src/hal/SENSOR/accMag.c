...
static inline unsigned char l_readI2cByte_ui8( unsigned char f_registerAddr_ui8 )
{
    ...
    if ( g_lldI2c_Write_bool(M_HAL_ACCMAG_I2CADDR_UI8, &l_registerAddr_ui8, 1) != M_HAL_ACCMAG_SUCCESS_BOOL )
    {
        // error in I2C access
        return M_HAL_ACCMAG_FAILED_BOOL;
    }

    //read requested register data
    if ( g_lldI2c_Read_bool(M_HAL_ACCMAG_I2CADDR_UI8, &l_recvDataByte_ui8, 1) != M_HAL_ACCMAG_SUCCESS_BOOL )
    {
        // error in I2C access
        return M_HAL_ACCMAG_FAILED_BOOL;
    }
...
static inline unsigned int l_readI2cByteStream_bool( unsigned char f_startRegisterAddr_ui8, const unsigned char* f_readDestinationBuffer_pui8, unsigned int f_numOfBytesToRead_ui32 )
{
    ...
    if ( g_lldI2c_Read_bool(M_HAL_ACCMAG_I2CADDR_UI8, f_readDestinationBuffer_pui8, f_numOfBytesToRead_ui32) != M_HAL_ACCMAG_SUCCESS_BOOL )
    {
        // error in I2C access
        return M_HAL_ACCMAG_FAILED_BOOL;
    }

    return M_HAL_ACCMAG_SUCCESS_BOOL;
}
...
//{
//    unsigned int    l_writeSucceeded_bool     = M_HAL_ACCMAG_SUCCESS_BOOL; // success state as default
...
//    if ( g_lldI2c_Write_bool(M_HAL_ACCMAG_I2CADDR_UI8, l_writeBuffer_rg2ui8, 2) == M_HAL_ACCMAG_FAILED_BOOL )
//    {
//        // error in I2C access
//        l_writeSucceeded_bool = M_HAL_ACCMAG_FAILED_BOOL;
//    }
//
//    return l_writeSucceeded_bool;
//}
//
//// END: DISABLED CODE
//
...
static inline unsigned int l_writeI2cByteStream_bool( unsigned char f_startRegisterAddr_ui8, const unsigned char* const f_byteStreamToWrite_pui8, unsigned int f_numOfBytes_ui32 )
{
    ...
    //write register to I2C
    if ( g_lldI2c_Write_bool(M_HAL_ACCMAG_I2CADDR_UI8, l_byteStreamBuffer_rgXui8, f_numOfBytes_ui32 + 1) != M_HAL_ACCMAG_SUCCESS_BOOL )
    {
        // error in I2C access
        l_writeSucceeded_bool = M_HAL_ACCMAG_FAILED_BOOL;
    }
...
static inline unsigned int l_updateSensorState_vd( halAccmag_sensorSelect f_sensor_en )
{
    ...
    unsigned int                l_successState_bool = M_HAL_ACCMAG_SUCCESS_BOOL;
    ...
    if ( (f_sensor_en == SENSOR_ACC_EN) || (f_sensor_en == SENSOR_BOTH_EN) )
    {
        if ( l_readI2cByteStream_bool(M_HAL_ACCMAG_OUT_X_L_A_UI8,l_readBuffer_rg6ui8, 6) == M_HAL_ACCMAG_SUCCESS_BOOL )
        {
            ...
            l_successState_bool = M_HAL_ACCMAG_FAILED_BOOL;
        }
