...
int g_halGyro_initGyro_i32(void)
{
    ...
    if(g_lldI2c_Write_bool(M_I2C_SLAVE_ADDRESS_GYRO_UI8, l_WriteBuffer_ui8, M_HAL_BARO_NR_OF_BYTES_THREE_UI8)!=0)
    {return -1;}
    ...
    if(g_lldI2c_Write_bool(M_I2C_SLAVE_ADDRESS_GYRO_UI8, l_WriteBuffer_ui8, M_HAL_BARO_NR_OF_BYTES_TWO_UI8)!=0)
    {return -1;}
 ...
 * \endinternal
 ********************************************************************** */
int g_halGyro_I2C_readRawValue_i32(void)
{
    ...
    if(g_lldI2c_Write_bool(M_I2C_SLAVE_ADDRESS_GYRO_UI8, l_WriteBuffer_ui8, M_HAL_BARO_NR_OF_BYTES_ONE_UI8)!=0)
    {return -1;}

    //Read from OUT_X_L (28h), OUT_X_H (29h), OUT_Y_L (2Ah), OUT_Y_H (2Bh), OUT_Z_L (2Ch), OUT_Z_H (2Dh)
    if(g_lldI2c_Read_bool(M_I2C_SLAVE_ADDRESS_GYRO_UI8, l_ReadBuffer_ui8, M_HAL_BARO_NR_OF_BYTES_SIX_UI8)!=0)
    {return -1;}
    else
    {
...
int g_halGyro_readTemperatureFromI2C_i32(void)
{
    ...
    if(g_lldI2c_Read_bool(M_I2C_SLAVE_ADDRESS_GYRO_UI8, l_ReadBuffer_ui8, M_HAL_BARO_NR_OF_BYTES_ONE_UI8)!=0)
    {return -1;}
    else
    {
